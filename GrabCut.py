...
maskers = 0
# 读入图片
img = cv2.imread('D:\PWG CUT\\train_image\\train003.jpg')
print(type(img))
#img = cv2.resize(img, dsize=(1920, 1080))
#cv2.imwrite('train_image/train012.jpg', (img))
...
during1 = (end1 - start) / cv2.getTickFrequency()
print(during1)
...
while cv2.waitKey(2) == -1:
    ...
    # 左键松开，矩形画好
        ...
        during2 = (end2 - start) / cv2.getTickFrequency()
        print(during2)

        # 物体分割

        cv2.grabCut(img, mask, rect_copy, bgdModel, fgdModel, 1, cv2.GC_INIT_WITH_RECT)
        maskers = np.where((mask == 2) | (mask == 0), 0, 1).astype('uint8')
        img_show = img * maskers[:, :, np.newaxis]
        # 显示图片分割后结果
        cv2.imshow('PWGCut', (img_show))
        #cv2.imwrite('test_data/test_data035_a.jpg', (img_show))
        # 显示原图

        cv2.imshow('img', img)
        end3 = cv2.getTickCount()
        during3 = (end3 - start) / cv2.getTickFrequency()
        print(during3)
        print(during3-during2+during1)
